...
def partition_indices_all_years(num_parts=20):
    ...
    index_starts = {}
    for yr in range(1999, 2019):
        df_yr = pd.read_csv(os.path.join(MODEL_BIN_DATA_PATH, 'm_b-preprocessed-{}.csv'.format(yr)), index_col='id')
...
def separate_years(binary=False, num_parts=20, year_1=1999, year_2=2019):
    ...
    for yr in range(year_1, year_2):
        if binary:
            ...
            df_yr = pd.read_csv(os.path.join(MODEL_DIS_DATA_PATH, 'm_d-preprocessed-{}.csv'.format(yr)), index_col='id')

        for j in range(num_parts):
            df_par_j = df_yr.loc[indices_by_yr_prts[j][yr]]
            df_par_j.dropna(inplace=True)
            if binary:
                df_par_j.to_csv(os.path.join(MODEL_BIN_COMB, 'm_b-preprocessed-part-{}-{}.csv'.format(j, yr)))
            else:
                df_par_j.to_csv(os.path.join(MODEL_DIS_COMB, 'm_d-preprocessed-part-{}-{}.csv'.format(j, yr)))
...
def combine_years(binary=False, year_1=1999, year_2=2019, part_1=0, part_2=20):
    ...
    for p in range(part_1, part_2):
        ...
        for yr in range(year_1, year_2):
            if binary:
                df_yr = pd.read_csv(os.path.join(MODEL_BIN_COMB, 'm_b-preprocessed-part-{}-{}.csv'.format(p, yr)),
                                    index_col='id')
            else:
                df_yr = pd.read_csv(os.path.join(MODEL_DIS_COMB, 'm_d-preprocessed-part-{}-{}.csv'.format(p, yr)),
                                    index_col='id')
            if df_par_p is None:
                df_par_p = df_yr
            else:
        ...
        if binary:
            df_par_p.to_csv(os.path.join(MODEL_BIN_COMB, 'm_b-preprocessed-part-{}.csv'.format(p)))
        else:
            df_par_p.to_csv(os.path.join(MODEL_DIS_COMB, 'm_d-preprocessed-part-{}.csv'.format(p)))


##### To preprocess for each year separately
